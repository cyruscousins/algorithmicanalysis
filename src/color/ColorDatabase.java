...
public class ColorDatabase {
	...
	public static void main(String[] args){
		...
		ColorDatabase mngr = new ColorDatabase();
		for(int i = 0; i < 20; i++){
			int r = rand.nextInt(255);
			int g = rand.nextInt(255);
			int b = rand.nextInt(255);
			System.out.println(r + " " + g + " " + b + ": " + mngr.nameColor(new Color(r, g, b)));
		}
	}
...
//			BufferedReader reader = new BufferedReader(new FileReader("res/color/xkcd.csv"));
			...
			while(reader.ready()){
				...
				if(split.length == 2){
					...
					int col = readHex(split[1]);
					
					//Convert bgr <--> rgb colorspace (not necessarily, already in rgb
					//col = ((col & 0x0000ff) << 16) | ((col & 0xff0000) >> 16) | (col & 0x00ff00);
...
//		}
	...
	public String nameColor(Color col){
		...
		for(int i = 0; i < colors.size(); i++){
			int newCol = colors.get(i);
			int r1 = (newCol >> 16) & 0xff;
			int g1 = (newCol >>  8) & 0xff;
			int b1 = (newCol >>  0) & 0xff;
			
			int distance = Math.abs(r0 - r1) + Math.abs(g0 - g1) + Math.abs(b0 - b1); 
		...
		return names.get(minDistanceIndex);
	}
	
	public int lookup(String s){
		//TODO table lookups plz.
	...
	public Color[] getRandomSelection(int count){
		Color[] c = new Color[count];
		for(int i = 0; i < count; i++) c[i] = new Color(colors.get(cRand.nextInt(names.size())).intValue());
		return c;
	}
}
